...
`InventoryApi/Http/DomainExceptionHandler.cs` maps both new types, plus the pre-existing exception types controllers already caught one by one (`ArgumentException`, `InvalidOperationException`, `InsufficientStockException`), to the exact status code and message each already produced: `DomainValidationException`/`ArgumentException`/`InvalidOperationException`/`InsufficientStockException` become `400 Bad Request`; `DomainConflictException` becomes `409 Conflict`. Centralizing the pre-existing types (rather than only the two new ones) is what lets a migrated controller drop its `try`/`catch` entirely without changing the status code or message a client already depends on. `InventoryCostDataQualityException` is deliberately excluded even though it derives from `InvalidOperationException`: it signals a data-integrity problem found while rebuilding costing history, not a routine validation failure, and nothing has ever caught it, so it keeps surfacing as an unexpected error rather than being silently reclassified as a client mistake. Every `ProblemDetails` response carries the current trace ID and a `message` extension equal to `detail`, kept for the existing frontend error handling that already reads `error.message` for this exact migration (see `machine-detail.component.ts`). A validation outcome is not logged; a conflict is logged once at `Warning` with the trace ID, because it may indicate a real race between two callers, but it is still an expected outcome rather than a server error.
