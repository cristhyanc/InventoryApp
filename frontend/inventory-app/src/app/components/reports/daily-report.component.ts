...
<tr class="border-t align-top">
  ...
  <td class="px-4 py-3"><span [class.text-amber-700]="row.isCogsComplete === false">{{ row.isCogsComplete === false ? 'Profit unavailable' : money(row.grossProfit) }}</span>@if (row.isCogsComplete !== false) {<div class="text-xs text-slate-500">{{ row.grossMarginPercent ?? 0 | number:'1.1-1' }}% margin</div>} @else {<div class="text-xs text-amber-700">COGS incomplete</div>}</td>
...
@if (report.totals) {
<tr class="border-t-2 bg-slate-50 font-semibold"><td class="px-4 py-3">Total</td><td class="px-4 py-3">{{ report.totals.transactionCount }}<div class="text-xs font-normal text-slate-500">Avg {{ money(report.totals.averageSale) }}</div></td><td class="px-4 py-3">{{ money(report.totals.grossSales) }}<div class="text-xs font-normal text-slate-500">Card {{ money(report.totals.cardSales) }} · Cash {{ money(report.totals.cashSales) }}</div></td><td class="px-4 py-3">{{ report.totals.isCogsComplete ? money(report.totals.costOfGoods) : 'Partial ' + money(report.totals.partialCostOfGoods) }}@if (!report.totals.isCogsComplete) {<div class="text-xs font-normal text-amber-700">Incomplete</div>}</td><td class="px-4 py-3">{{ report.totals.isCogsComplete ? money(report.totals.grossProfit) : 'Profit unavailable' }}@if (report.totals.isCogsComplete) {<div class="text-xs font-normal text-slate-500">{{ report.totals.grossMarginPercent | number:'1.1-1' }}%</div>} @else {<div class="text-xs font-normal text-amber-700">Incomplete</div>}</td><td class="px-4 py-3">{{ money(report.totals.nayaxFeesIncludingGst) }} fees<div class="text-xs font-normal text-slate-500">{{ money(report.totals.importedReimbursement) }} reimbursement · net {{ money(report.totals.netReimbursement) }}</div></td><td class="px-4 py-3">—</td></tr>
}
</tbody></table></div>
}
` })
