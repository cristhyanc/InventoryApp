...
@if (loading) { <div class="rounded-xl bg-white p-8 text-center text-slate-500">Loading report...</div> } @else if (error) { <div class="rounded-xl bg-red-50 p-6 text-red-700">{{ error }}</div> } @else if (report) { <div class="overflow-x-auto rounded-xl bg-white shadow-sm"><table class="min-w-full text-sm"><thead><tr class="bg-slate-50 text-left"><th class="px-4 py-3">Machine</th><th class="px-4 py-3">Commission</th><th class="px-4 py-3">Nayax Fees</th><th class="px-4 py-3">Direct Expenses</th><th class="px-4 py-3">Transactions</th><th class="px-4 py-3">Sales</th><th class="px-4 py-3">COGS</th><th class="px-4 py-3">Gross Profit</th><th class="px-4 py-3">Net Profit</th><th class="px-4 py-3">Margin</th></tr></thead><tbody>@for (row of report.rows; track row.machineId) {<tr class="border-t"><td class="px-4 py-3">{{ row.machineName }}</td><td class="px-4 py-3">{{ (row.commissionPercent ?? 0) * 100 | number:'1.2-2' }}% ({{ money(row.siteCommission) }})</td><td class="px-4 py-3">{{ money(row.nayaxProcessingFees?.totalFeeIncGst) }}@if (row.nayaxProcessingFees?.hasEstimatedFees) {<div class="text-xs text-amber-700">Includes estimated fees</div>}</td><td class="px-4 py-3">{{ money(row.directOperatingExpenses) }}</td><td class="px-4 py-3">{{ row.transactionCount }}</td><td class="px-4 py-3"><div>{{ money(row.sales) }}</div><div class="text-xs text-slate-500">Card {{ money(row.cardSales) }} · Cash {{ money(row.cashSales) }}</div></td><td class="px-4 py-3">{{ row.isCogsComplete === false ? 'Partial ' + money(row.partialCostOfGoods) : money(row.costOfGoods) }}</td><td class="px-4 py-3">{{ moneyOrUnavailable(row.grossProfit) }}</td><td class="px-4 py-3">{{ moneyOrUnavailable(row.netProfit) }}</td><td class="px-4 py-3">{{ row.netMarginPercent == null ? 'Profit unavailable' : (row.netMarginPercent | number:'1.2-2') + '%' }}</td></tr>}</tbody></table></div> }
` })
