...
<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
  ...
  <div class="rounded-xl bg-white p-5 shadow-sm"><div class="text-2xl font-semibold text-blue-600">{{ moneyOrUnavailable(report.grossProfit) }}</div><div class="mt-1 text-sm text-slate-500">Gross Profit</div><div class="mt-2 text-xs text-slate-500">{{ report.grossMarginPercent == null ? 'COGS incomplete' : (report.grossMarginPercent | number:'1.1-1') + '% margin' }}</div></div>
  <div class="rounded-xl bg-emerald-50 p-5 shadow-sm"><div class="text-2xl font-semibold text-emerald-700">{{ moneyOrUnavailable(machineId ? report.directProfit : report.netProfit) }}</div><div class="mt-1 text-sm text-emerald-800">{{ machineId ? 'Direct Profit' : 'Net Profit' }}</div><div class="mt-2 text-xs text-emerald-700">{{ (machineId ? report.directMarginPercent : report.netMarginPercent) == null ? 'Profit unavailable' : ((machineId ? report.directMarginPercent : report.netMarginPercent) | number:'1.1-1') + '% margin' }}@if (report.nayaxProcessingFees?.hasEstimatedFees) { · Includes estimated fees}</div></div>
...
<div class="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
  ...
  <div class="rounded-lg bg-slate-50 p-4"><div class="text-lg font-semibold text-slate-700">{{ report.isCogsComplete === false ? 'Partial ' + money(report.partialCostOfGoods) : money(report.costOfGoodsSold) }}</div><div class="text-xs text-slate-500">Cost of Goods Sold</div></div>
