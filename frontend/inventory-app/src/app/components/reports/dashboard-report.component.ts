...
<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
  ...
  <div class="rounded-xl bg-emerald-50 p-5 shadow-sm"><div class="text-2xl font-semibold text-emerald-700">{{ money(report.netProfit) }}</div><div class="mt-1 text-sm text-emerald-800">Net Profit</div><div class="mt-2 text-xs text-emerald-700">{{ report.netMarginPercent | number:'1.1-1' }}% margin@if (report.nayaxProcessingFees?.hasEstimatedFees) { · Includes estimated fees}</div></div>
