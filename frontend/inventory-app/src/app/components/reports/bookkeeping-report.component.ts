  ...
  template: `
    ...
    @else if (report) {
      <div class="mb-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><div class="text-xs font-semibold uppercase tracking-wide text-slate-500">Gross Profit</div><div class="mt-2 text-2xl font-bold text-slate-800">{{ money(report.grossProfit) }}</div><div class="mt-1 text-sm text-slate-500">{{ margin(report.sales, report.grossProfit) | number:'1.1-1' }}% margin</div></div>
        <div class="rounded-xl border-2 border-emerald-500 bg-emerald-50 p-4 shadow-sm"><div class="text-xs font-semibold uppercase tracking-wide text-emerald-700">Net Profit</div><div class="mt-2 text-3xl font-bold text-emerald-800">{{ money(report.netProfit) }}</div></div>
        <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><div class="text-xs font-semibold uppercase tracking-wide text-slate-500">Net Margin</div><div class="mt-2 text-2xl font-bold text-slate-800">{{ report.netMarginPercent | number:'1.1-1' }}%</div></div>
      </div>
      <div class="grid gap-5 lg:grid-cols-2">
        <section class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><h2 class="mb-4 text-lg font-semibold text-slate-800">Cost of Sales</h2><div class="space-y-3 text-sm"><div class="flex justify-between"><span>Gross Sales</span><strong>{{ money(report.sales) }}</strong></div><div class="flex justify-between pl-4 text-slate-500"><span>Card Sales</span><strong>{{ money(report.cardSales) }}</strong></div><div class="flex justify-between pl-4 text-slate-500"><span>Cash Sales</span><strong>{{ money(report.cashSales) }}</strong></div><div class="flex justify-between"><span>COGS</span><strong>{{ money(report.costOfGoods) }}</strong></div><div class="flex justify-between border-t border-slate-100 pt-3"><span class="font-semibold">Gross Profit</span><strong>{{ money(report.grossProfit) }}</strong></div><div class="flex justify-between text-slate-500"><span>Gross Margin</span><strong>{{ margin(report.sales, report.grossProfit) | number:'1.1-1' }}%</strong></div></div></section>
        <section class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><h2 class="mb-4 text-lg font-semibold text-slate-800">Operating Costs</h2><div class="space-y-3 text-sm"><div class="flex justify-between"><span>Nayax Fees Inc GST</span><strong>{{ money(report.nayaxFeesIncludingGst) }}</strong></div><div class="pl-2 text-xs text-slate-500">{{ (report.nayaxProcessingRate ?? 0) | number:'1.1-1' }}% of card sales</div><div class="flex justify-between"><span>Site Commission</span><strong>{{ money(report.siteCommission) }}</strong></div><div class="flex justify-between"><span>Delivery Costs</span><strong>{{ money(report.deliveryCosts) }}</strong></div><div class="flex justify-between"><span>Package Costs</span><strong>{{ money(report.packageCosts) }}</strong></div><div class="flex justify-between"><span>Other Operating Expenses</span><strong>{{ money(otherExpenses(report)) }}</strong></div><div class="flex justify-between border-t border-slate-200 pt-3 text-base"><span class="font-semibold">Total Operating Costs</span><strong>{{ money(totalOperatingCosts(report)) }}</strong></div></div></section>
...
export class BookkeepingReportComponent extends ReportPageBase<BookkeepingReport> {
  ...
  margin(sales: number, profit: number): number { return sales ? profit / sales * 100 : 0; }
  otherExpenses(report: BookkeepingReport): number { return Math.max(0, (report.otherOperatingExpenses ?? 0) - (report.deliveryCosts ?? 0) - (report.packageCosts ?? 0)); }
  totalOperatingCosts(report: BookkeepingReport): number { return (report.nayaxFeesIncludingGst ?? 0) + (report.siteCommission ?? 0) + (report.otherOperatingExpenses ?? 0); }
}
