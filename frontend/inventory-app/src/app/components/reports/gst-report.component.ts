...
@if (loading) { <div class="rounded-xl bg-white p-8 text-center text-slate-500">Loading report...</div> } @else if (error) { <div class="rounded-xl bg-red-50 p-6 text-red-700">{{ error }}</div> } @else if (report) { <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">@for (card of [['Taxable sales',report.taxableSales],['GST on sales',report.gstOnSales],['Taxable fees',report.taxableFees],['GST on fees',report.gstOnFees],['Estimated GST payable',report.netGst]]; track card[0]) {<div class="rounded-xl bg-white p-5 shadow-sm"><div class="text-xl font-semibold">{{ money($any(card[1])) }}</div><div class="mt-1 text-sm text-slate-500">{{ card[0] }}</div></div>}</div> }
` })
